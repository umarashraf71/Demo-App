          ...
          <form class="form" action="{{ route('section.store') }}" method="POST">
            ...
            <div class="row">
              <div class="col-md-6 col-12">
                <div class="mb-1">
                  <label class="form-label" for="dept-code-column">Section Code *</label>
                  <input
                    type="text"
                    id="dept-code-column"
                    class="form-control"
              ...
              <div class="col-md-6 col-12">
                <div class="mb-1">
                    <label class="form-label" for="select2-multiple">Select Department *</label>
                    <select
                        class="select2 form-select"
                        id="select2-multiple"
                        name="dept_id"
                    >
                        <option value="" selected disabled>Select Deptartment</option>
                        @foreach ($all_dept as $value)
                        <option
                        value="{{ $value->id }}"
                        {{ (Input::old("dept_id") == $value->id  ? "selected":"") }}
                        >{{ $value->name }}</option>
                        @endforeach
                    </select>
                    @error('dept_id')
